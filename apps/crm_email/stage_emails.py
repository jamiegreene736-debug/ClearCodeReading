...
def template_key(
    deal: Opportunity | None, delivery: StageEmailDelivery | None = None
) -> str:
    ...
    if deal is None:
        raise EmailError("This delivery has no deal and no email template.")
    return "stage_" + deal.pipeline


def render_copy(
    deal: Opportunity | None,
    pilot: StageEmailPilot,
    key: str = "",
    lead: Lead | None = None,
) -> StageCopy:
    ...
    copy = copy_for(key)
    contact = lead if lead is not None else (deal.lead if deal else None)
    pipeline = deal.pipeline if deal else ""
    name = (
    ...
    values = {
        ...
        or plain_text(sending_mailbox(pipeline, pilot).signature),
    }
    labels = {
    ...
    if copy.customized:
        ...
        source = str(first_stage_source()[pipeline]["source"])
...
def stage_send_allowed(message: Message) -> bool:
    delivery = (
        StageEmailDelivery.objects.select_related(
            "deal", "lead", "pilot__mailbox__user", "pilot__equity_mailbox__user"
        )
    ...
    try:
        ...
        with (
            ...
            copy = render_copy(
                delivery.deal,
                pilot,
                template_key(delivery.deal, delivery),
                lead=delivery.lead,
            )
            if copy.missing:
